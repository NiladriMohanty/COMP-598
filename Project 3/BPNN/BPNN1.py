...
#This part is used to train the data set

import numpy as np
import re
import csv

if_output=1 #Used to control output
...
# Load all training outputs to a python list
train_outputs = []
...
#learning rate
lr_in_hide=0.8; #learning rate for the weights between input layer and hidden layer
lr_hide_out=0.8; #learning rate for the weights between hidden layer and output layer
lr_hide_sigmoid=0.8; #learning rate for the sigmoid output of hidden layer
lr_out_sigmoid=0.8; #learning rate for the sigmoid output of output layer

#value initialization            
w_in_hide = (np.random.rand(inode,hn)-1)/400000 #the weight between input layer and hidden layer
w_hide_out = (np.random.rand(hn)-1)/400000 #the weight between hidden layer and output layer
threshold1=(np.random.rand(hn)-1)/400000 #the sigmoid value of hidden layer
threshold2=((np.random.rand(1))[0]-1)/40000 #the sigmoid value of output layer
...
def main(run):
    ...
    #As these variables keep updating with iterations
    #Need to claim them as global variables
    global error
    global e
    global w_in_hide
    global w_hide_out
...
if if_output == 1:    
    with open("w_in_hide.csv", "wb") as f0:
        writer = csv.writer(f0)
        writer.writerows(w_in_hide)
    f0.close()
    
    with open("w_hide_out.csv", "wb") as f1:
        writer = csv.writer(f1)
        writer.writerow(w_hide_out)
    f1.close()
    
    with open("threshold1.csv", "wb") as f2:
        writer = csv.writer(f2)
        writer.writerow(threshold1)
    f2.close()
    
    f3=open('threshold2.csv','w')
    f3.write(str(threshold2))
    f3.close()
